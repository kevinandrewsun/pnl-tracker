...
const fetchTradesData = async (ticker) => {
  ...
  const cumExp = useMemo(() => { if (!selDate || !hist.length) return null; const fd = hist.filter(h => h.date <= selDate); if (!fd.length) return null; const n = fd.length; let tL = 0, tS = 0, tC = 0; fd.forEach(h => { const e = h.entries, l = e.filter(x => x.dir === "L").reduce((s, x) => s + x.size, 0), sh = e.filter(x => x.dir === "S").reduce((s, x) => s + x.size, 0); tL += l; tS += sh; tC += e.length; }); return { long: tL / n, short: tS / n, net: (tL - tS) / n, gross: (tL + tS) / n, count: tC / n, days: n }; }, [hist, selDate]);
  ...
  return (
    <div style={S.bg}><div style={{ maxWidth: 1060, margin: "0 auto" }}>
      ...
      {tab === 0 && <div>
        ...
        {manRet && <div style={{ ...S.card, marginBottom: 16, borderColor: "#ef4444" }}><h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: 8 }}>Missing Returns</h3><p style={{ fontSize: 12, color: "#71717a", marginBottom: 12 }}>Enter %. Negatives: -1.3 or (1.3%)</p>{failTk.map(t => <div key={t} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}><span style={{ fontWeight: 600, fontSize: 13, minWidth: 90 }}>{t}</span><input value={manRet[t] || ""} onChange={e => setManRet(p => ({ ...p, [t]: e.target.value }))} placeholder="0.0" style={{ ...S.inp, width: 100, padding: "4px 8px" }} /><span style={{ fontSize: 12, color: "#71717a" }}>%</span></div>)}<div style={{ display: "flex", gap: 8, marginTop: 8 }}><button onClick={submitMan} style={S.btn}>Submit</button><button onClick={async () => { setFetching(true); try { const empty = failTk.filter(t => !manRet[t] || manRet[t].trim() === ""); if (!empty.length) { setMsg("All returns already filled in."); setFetching(false); return; } const returns = await fetchDailyReturns(empty, date); const updated = { ...manRet }; let count = 0; Object.entries(returns).forEach(([tk, r]) => { updated[tk] = (r * 100).toFixed(4); count++; }); setManRet(updated); setMsg(`Fetched ${count}/${empty.length} missing return(s).`); } catch (e) { setMsg("Failed to fetch: " + e.message); } setFetching(false); }} disabled={fetching} style={{ ...S.btn, background: "#f59e0b", color: "#0f1117", opacity: fetching ? 0.5 : 1 }}>{fetching ? "Fetching..." : "Auto-Fetch Missing"}</button></div></div>}
      </div>}
      ...
      {tab === 2 && (() => {
        ...
        return <div>{dec.length === 0 ? <div style={{ ...S.card, textAlign: "center", padding: 40, color: "#71717a" }}>Import baseline + one day with returns.</div> : <div style={S.card}>
          ...
          {!dD ? <p style={{ color: "#71717a" }}>No decomposition.</p> : <div style={{ maxHeight: "calc(100vh - 260px)", overflowY: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse" }}><thead style={{ position: "sticky", top: 0, zIndex: 1, background: "#1a1d27" }}><tr>{st.th("Ticker","ticker")}{st.th("Side","dir")}{st.th("Sector","sector")}{st.th("Prior","priorSize",true)}{st.th("Size","size",true)}{st.th("Return","dailyReturn",true)}{st.th("P&L","pnl",true)}{st.th("Trading","trading",true)}</tr></thead><tbody>
            ...
            <tr style={{ borderTop: "1px solid #2a2d3a" }}><td style={{ ...S.td, fontWeight: 700 }}>TOTAL</td><td style={S.td}></td><td style={S.td}></td><td style={S.td}></td><td style={S.td}></td><td style={S.td}></td><td style={{ ...S.td, textAlign: "right", color: colF(tP), fontWeight: 700 }}>{fB(tP)}</td><td style={{ ...S.td, textAlign: "right", color: colF(tT), fontWeight: 700 }}>{fB(tT)}</td></tr>
      ...
      {tab === 1 && <div>{!latH ? <div style={{ ...S.card, textAlign: "center", padding: 40, color: "#71717a" }}>No data yet.</div> : <>
        ...
        {cumExp && <div style={{ marginBottom: 20 }}><h3 style={{ fontSize: 13, color: "#71717a", marginBottom: 8 }}>AVG EXPOSURE — {cumExp.days}d through {selDate}</h3><Cards items={[{ l: "Avg Long", v: cumExp.long, w: true }, { l: "Avg Short", v: cumExp.short, w: true }, { l: "Avg Net", v: cumExp.net, w: true }, { l: "Avg Gross", v: cumExp.gross, w: true }, { l: "Avg Positions", v: +cumExp.count.toFixed(1), n: true }]} /></div>}
      ...
      {tab === 8 && (() => {
        ...
        return <div style={S.card}>
          ...
          {cumTk.length === 0 ? <p style={{ color: "#71717a" }}>No data.</p> : <div style={{ maxHeight: "calc(100vh - 260px)", overflowY: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse" }}><thead style={{ position: "sticky", top: 0, zIndex: 1, background: "#1a1d27" }}><tr>{st.th("Ticker","ticker")}{st.th("Side","dir")}{st.th("Sector","sector")}{st.th("Last P&L","lastPnl",true)}{st.th("Cum P&L","cumPnl",true)}{st.th("Cum Trd","cumTr",true)}{st.th("Avg Wt","avgWt",true)}{st.th("ROIC","roic",true)}</tr></thead><tbody>
      ...
      {tab === 3 && (() => {
        ...
        return <div style={S.card}>
          ...
          {weeklyTk.length === 0 ? <p style={{ color: "#71717a" }}>No data.</p> : <div style={{ overflowX: "auto" }}><div style={{ maxHeight: "calc(100vh - 260px)", overflowY: "auto" }}><table style={{ minWidth: 500 + weeks.length * 80 }}><thead style={{ position: "sticky", top: 0, zIndex: 1, background: "#1a1d27" }}><tr>
      ...
      {tab === 4 && (() => {
        ...
        return <div style={S.card}>
          ...
          {monthlyTk.length === 0 ? <p style={{ color: "#71717a" }}>No data.</p> : <div style={{ overflowX: "auto" }}><div style={{ maxHeight: "calc(100vh - 260px)", overflowY: "auto" }}><table style={{ minWidth: 500 + months.length * 100 }}><thead style={{ position: "sticky", top: 0, zIndex: 1, background: "#1a1d27" }}><tr>
            <th onClick={() => toggle("ticker")} style={{ ...S.th, cursor: "pointer", userSelect: "none", position: "sticky", left: 0, background: "#1a1d27", zIndex: 2 }}>Ticker{arr("ticker")}</th>
            <th style={S.th}>Side</th><th onClick={() => toggle("sector")} style={{ ...S.th, cursor: "pointer", userSelect: "none" }}>Sector{arr("sector")}</th>
